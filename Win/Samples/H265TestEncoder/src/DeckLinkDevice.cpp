...
void BSTRToQString(BSTR strIn, QString& qStringOut)
{
	_bstr_t myBStr(strIn, false);
	wchar_t* wstr = myBStr;
	qStringOut = QString::fromUtf16((const ushort*)wstr);
}
...
bool DeckLinkDevice::init()
{
	...
	BSTRToQString(devName, m_deviceName);
...
QString DeckLinkDevice::getDisplayModeName(int displayModeIndex)
{
	...
	BSTR displayModeName = NULL;
	if(m_modeList[displayModeIndex]->GetName(&displayModeName) != S_OK)
		return QString();

	QString name;
	BSTRToQString(displayModeName, name);
	if(displayModeName)
...
HRESULT DeckLinkDevice::AudioPacketArrived(IDeckLinkEncoderAudioPacket* audioPacket)
{
	return S_OK;
}
...
{
	HRESULT result = CoCreateInstance(CLSID_CDeckLinkDiscovery, NULL, CLSCTX_ALL, IID_IDeckLinkDiscovery, (void**)&deckLinkDiscovery);
	if(result != S_OK)
		printf("Failed to create CDeckLinkDiscovery, error is: %d", result);
}
